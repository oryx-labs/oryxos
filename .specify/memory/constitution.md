...
- Bump rationale: First formal adoption of the OryxOS constitution. MAJOR baseline.
...
- 语言 / 运行时：Java 21（虚拟线程），框架 Spring Boot 3.x，构建 Maven 多模块（9 模块）。
...
- 开放标准优先：工具用 MCP、Agent 协作用 A2A、技能用 `SKILL.md`，不另立协议。
