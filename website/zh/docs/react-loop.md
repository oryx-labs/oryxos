...
7. **有工具调用** → `ToolExecutor` 执行工具，结果以结构化的工具消息回填历史，回到第 3 步
...
Agent 目录里的子指令（`skills/*.md`）和脚本（`scripts/`）**不会**被预先注入——正文会指引模型按需用 `read_file` / `shell` 读取。
...
6. **返回** — `ToolResult` 作为带原始工具调用 id 的结构化工具结果消息追加到对话历史，让下一次 LLM 调用把它当作正规的工具响应，而不是自由文本
