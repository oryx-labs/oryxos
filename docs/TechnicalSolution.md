...
1. system prompt（Profile 的 identity prompt 加 Bootstrap 文件加 `<available_skills>` 能力索引（L1：全部已装能力的 name+description，正文经 `use_skill` 按需加载），由 `ContextLoader` 提供；末尾附当前日期时间——LLM 自己不知道今天几号，定时场景的"今天"全靠这一行）
...
>
> 另外，`SKILL.md` 严格说不是一种 Tool，而是一项可复用能力（L1 元数据注入 system prompt、正文经 `use_skill` 按需加载，见 11.1），因此 `SkillLoader` 不放在 Tool 体系里，归到上下文加载那一层（见 8.3）。这样概念更齐整。
...
一个 Skill 是 `.oryxos/skills/<name>/` 一个**目录**（Anthropic Agent Skills 完整形态，详见第 11 章）：`SKILL.md` = frontmatter（**只有 `name` + `description`**）加任务说明正文，外加可选的 `scripts/*.py`、`REFERENCE.md` 等资源。它是一项**可复用能力**，被任意 Agent 按需加载，不绑某个 Profile。

加载走**渐进式披露**：全部已装能力的 `name`+`description`（L1）进 system prompt，模型判断相关了才调 `use_skill` 取正文（L2），正文里的脚本 / 参考再按需读跑（L3）。LLM 读到后自己理解任务、自己决定调哪个 MCP 工具、自己组合完成——OryxOS 不解析任务步骤、不做工作流引擎，所有逻辑交给 LLM。

> 注意 `SKILL.md` 的解析由 `SkillLoader` / `ContextLoader`（8.3）负责，不在 Tool 模块里——它是 prompt 的输入源、不是可执行 Tool（`use_skill` 返回的是指令内容，见宪法原则四与 11.1）。
...
### 8.3 上下文加载（Bootstrap + Skill 索引）

> **调整说明：** Bootstrap 文件与 Skill 能力索引都由 `ContextLoader` 提供给 system prompt，只是 Bootstrap 是全量注入、Skill 走**渐进式披露**（详见 11.1）：能力的 L1 元数据（`name`+`description`）进 system prompt，L2 正文经 `use_skill` 按需加载。

**`ContextLoader` 模块。** 按 Profile 的 `bootstrap` 字段从 `.oryxos/` 读取 `AGENTS.md`、`SOUL.md`、`USER.md`（Bootstrap，全量注入）；同时把 `SkillRegistry` 里**全部已装能力**的 `name`+`description` 拼成 `<available_skills>` 索引（L1）一并交给 `PromptBuilder`。每次组装 prompt 时重新加载不缓存，用户修改后立即生效。能力**正文**不在这里注入，由 `use_skill` 工具在模型需要时现取（L2）——把 `SKILL.md` 归到上下文这一层而不是 Tool 模块，是因为它是 prompt 的输入、不是可执行的 Tool。
...
**核心阶段 vs 扩展阶段的边界。** 核心阶段的 `schedules` **定义**只能写在 Profile/Skill 里，跟着进程启动一起注册，改 cron / 新增任务要重启（或触发重新加载）才生效；第 28 节补齐的是任务的**状态持久化与运行控制**（查看 / 执行历史 / 立即执行 / 启用停用），不含通过 API 增删改 cron 定义。"业务方通过 Web Service 上传一份 SKILL.md、创建一个引用它的 Profile（带 `schedules` 字段）、由此定义一个新 Agent 并让它定时自动运行"这个完整闭环，依赖的是 7.3 里扩展阶段才补齐的三个能力——Skill 上传接口、Profile 创建接口、`AgentScheduler` 的运行时增删接口——核心阶段这条链路要靠手动编辑文件走通，扩展阶段补上这三个接口后才是纯 API、免重启的闭环。
...
| 模块名 | 职责 |
|--------|------|
| `oryxos-core` | 核心抽象和接口：`OryxTool` 接口、`Session`、`Profile`、`ContextLoader`、`SkillLoader`/`SkillRegistry`（能力库）、`ReActLoop`、`PromptBuilder`、`ToolExecutor`、`AgentService`、`AgentScheduler`（定时触发）、`AgentLifecycleService`/`SkillLifecycleService`（扩展阶段，编排"定义一个 Agent（Profile 落盘 + 注册 + Scheduler）"与"装一个能力（Skill 目录落盘 + SkillRegistry）"） |
...
## 11. 定义一个 Agent：Agent（Profile）+ Skill（可复用能力）+ Web Service

前面十章是**底座**——让任意 Agent 都能可靠运行的引擎、能力、支撑设施，本身不是某个具体的业务 Agent。这一章讲底座之上怎么真正"定义出一个业务 Agent"、怎么给它装可复用的能力，以及这两个动作通过哪个入口对外暴露。形态对齐 **Anthropic Agent Skills**。

### 11.1 术语：Agent 是 Profile，Skill 是能力，两者分离

**底座 / Agent / Skill 三层，分清楚。** 这是这一章最关键的一条：

- **Agent = 一份 Profile**：`identity`（人格 + 指令）、`provider`/`model`、`tools` 白名单、`notify_channels`、`schedules`（什么时候自己跑）。是"谁 / 何时 / 以什么身份跑"。底座（第 1~10 章的一切）都吃 `Profile`，这一层不变。
- **Skill = 一个能力目录**（Anthropic Agent Skills 完整形态）：`SKILL.md` = frontmatter（**只有 `name` + `description`**，即 L1 元数据）+ 正文（L2 指令）；外加可选 L3 资源（`scripts/*.py`、`REFERENCE.md`、模板）。是"一项**可复用**的专长"，被任意 Agent 按需加载。**能力里不含定时 / provider / 身份**——那些是 Agent 的属性。
- **两个注册表**：`SkillRegistry`（能力库，扫 `.oryxos/skills/`）与 `ProfileRegistry`（Agent，扫 `.oryxos/profiles/`）。**一个能力被 N 个 Agent 复用。**

**渐进式披露（progressive disclosure）——三级按需加载**，是"完整形态"的核心机制：

- **L1（永远）**：`ContextLoader`/`PromptBuilder` 把**全部**已装能力的 `name: description` 作为 `<available_skills>` 索引注入 system prompt。装再多能力，平时也只占这一小段。
- **L2（触发才加载）**：模型按 `description` 判断某能力相关 → 调底座通用工具 **`use_skill(name)`** 取回该能力的**正文**，作为工具结果进上下文。
- **L3（用到才加载）**：正文里指向的 `scripts/*.py`、`REFERENCE.md`，模型用 `read_file` 读、用 `shell`/`python` 跑——脚本产出结果进上下文、代码不进。

> **底线不变（宪法原则四，措辞已精确化）**：Skill 的 **L1 元数据由 `ContextLoader`/`PromptBuilder` 注入 system prompt**；**L2 正文改为经 `use_skill` 按需加载**（不再全量塞进 system prompt）。`use_skill` 返回的是**指令内容**、不是"执行一个 Skill"——**Skill 永远不是一个可执行 Tool**。`SkillLoader` 仍归上下文加载那一层（8.3），不进 Tool 体系。

### 11.2 核心阶段：装能力 + 定义 Agent（文件系统）

核心阶段走文件系统，两类资源各就各位：

1. **装能力**：在 `.oryxos/skills/<name>/` 放一个目录（`SKILL.md` + 可选 `scripts/`）→ 启动时 `SkillLoader` 解析、`SkillRegistry.register`。
2. **定义 Agent**：在 `.oryxos/profiles/<name>.yaml` 写一份 Profile（identity / provider / tools / notify / schedules）→ 启动时 `ProfileLoader` 加载、`ProfileRegistry.register`；有 `schedules` 的交 `AgentScheduler`。
3. **运行时**：Agent 到点被触发，看 system prompt 里的能力索引、`use_skill` 按需把能力正文加载进来照做；`ContextLoader` / `use_skill` 每次现取、不缓存，改能力正文即时生效。

配合运行时注册（11.3 的改造点在核心阶段就立好），新增能力 / Agent 无需重启。

### 11.3 扩展阶段：`/api/v1/skills` + `/api/v1/agents` + 一句话生成

业务系统 / 运营要完全通过 API 或页面管理、不摸文件系统，需要把两类资源各包成一组端点——**别糊成一个 `/agents` 全包**（Skill 是被 N 个 Agent 共享的能力，Agent 是"谁 / 何时"，混在一起等于把刚拆干净的两件事焊回去）。

**能力端点**（`SkillLifecycleService`）：`POST /api/v1/skills`（上传目录 → 校验 → `SkillRegistry.register`）、`GET`（查能力库）、`DELETE`（移除 + 归档）、`POST /api/v1/skills/generate`（一句话 → `SKILL.md` 草稿，只生成不落盘）。

**Agent 端点**（`AgentLifecycleService`，在 26 节已有的 `AgentApiController` 上扩展）：

- `POST /api/v1/agents/generate`：一句话经 LLM 生成一份 **Profile 草稿**原样返回（不落盘、不注册），供页面预览、修改（尤其 cron/tools 敏感项要人过一眼）
- `POST /api/v1/agents`：body 为 Profile 字段（**不含 skills 白名单**——全体能力共享；`tools` 里带上 `use_skill` 才能加载能力）——写 Profile YAML，再注册
- `GET /api/v1/agents` / `GET /{name}`：查询已定义的 Agent
- `PUT /api/v1/agents/{name}`：更新身份 / provider / notify（覆写即时生效）和/或 `schedules`（变则先注销旧句柄再注册新的）
- `DELETE /api/v1/agents/{name}`：注销定时 → 移出索引 → Profile 归档 `.oryxos/archive/`（不物理删）
- `POST /api/v1/agents/{name}/invoke`：已有的无状态调用端点，不变

![两条录入路径一段注册代码：API 上传 create() 与手工丢文件 WorkspaceWatcher 都汇到 register(file)，校验 + 注册 + 注册定时，免重启即上线](../website/public/images/docs-agent-lifecycle.svg)

写完 Profile YAML 后走的 `ProfileLoader.load → ProfileRegistry.register → AgentScheduler.registerProfile`，与启动扫描是**同一段代码**——保证"API 建的 Agent 和手写文件建的 Agent 行为一模一样"。`ProfileRegistry`（`register`/`remove`/`exists`）和 `AgentScheduler`（`registerProfile`/`unregisterProfile` + `scheduledTasks` 句柄表）的运行时注册方法在核心阶段（课程第 29 节）就已立好，本阶段直接调；`generateAgent`/`generateSkill` 走既有 `ProviderService`（并落 `llm_calls` 审计）。

**一个目录、两条录入路径 + 实时监听。** 工作区目录（`.oryxos/profiles/` 与 `.oryxos/skills/`）是**唯一真相源**，填充它有两条路且殊途同归：API 上传（校验 + 写文件进目录）、手工丢文件（scp/git/编辑器）。本阶段新增 `WorkspaceWatcher`（装配层一个守护线程，用 JDK `WatchService`；启动全量扫 + 之后实时监听变更），**它是统一注册入口**：任何文件新增/改/删都调 `AgentLifecycleService.register(file)`（与 API 上传写完文件后调的是**同一个方法**）或注销。于是"上传即上线 = 丢文件即上线、全程免重启"。此外 `WorkspaceApiController` 提供**只读**的工作区文件浏览（`GET /workspace/tree` 列目录树、`GET /workspace/file?path=` 读文件内容，**必做防目录穿越**：`normalize()` 后 `startsWith(root)` 校验），供管理台的"工作区"页当文件浏览器用。

### 11.4 为什么这几件事要打包在一起交付

单独做任何一件都拼不出"说一句话 / 调一次 API 就装一个能力、或上线一个会自动定时运行的新 Agent"这个闭环：少了能力上传，能力库进不了新专长；少了 Profile 创建，Agent 定义不了；少了运行时注册，新东西要等重启；少了定时注册，"会自己跑"是空话；少了一句话生成，运营还得手写 YAML / frontmatter。所以放在同一批一起交付，不拆开先做一半。

### 11.5 两个例子

下一章（12.1~12.3）的三个 Demo——每日天气、每日科技日报、每日 GitHub 日报——各演一个核心概念、正好覆盖 Skill 渐进式披露的三级：**天气 Agent 是纯 Profile**（指令写在 `identity.prompt`，用内置 HTTP Tool + `schedules`，不用能力）；**科技日报 Agent 是 Profile + 能力正文 L2**（装 `tech-digest` 能力，到点 `use_skill` 加载组稿指令，配 MCP / Memory）；**GitHub 日报 Agent 是 Profile + 能力捆绑脚本 L3**（装 `github-digest` 能力，含 `scripts/github_trending.py`，Agent 跑脚本拿确定性数据）。核心阶段先靠 11.2 的手动路径跑通，扩展阶段升级到 11.3 的统一 API 后，不用重启进程就能上新能力 / 新 Agent。
...
早期按"一个 Demo 验证一个能力"拆过五个流程，但真实场景里能力从来不是分开跑的。改成三个**每日自动运行**的端到端流程，横向串起全部五大核心能力加定时任务这个第三触发源——而且它们正好是第 11 章 Agent（Profile）/ Skill（能力）分离的具体产出，把 Skill 渐进式披露的三级（L1/L2/L3）各演到一个：天气 = 纯 Profile（不用能力）、科技日报 = Profile + 能力正文（L2）、GitHub 日报 = Profile + 能力捆绑脚本（L3）。底座和"定义 Agent / 装能力"两部分在这里合到一起跑通。

### 12.1 Demo 一：每日天气（纯 Profile）
...
2. `ReActLoop` 第一轮，`PromptBuilder` 通过 `ContextLoader` 组装 system prompt（Profile 的 `identity.prompt` + Bootstrap + `<available_skills>` 能力索引 L1——本 Agent 不用能力，索引在但不触发）
...
5. 结果追加到 Session 进入第二轮，DeepSeek 看到天气生成穿搭建议，决定调 `notify(content="...")` 推送——地址是 Profile 的 `notify_channels` 字段配置好的
...
**验收要点：** 全程不需要人工触发；两次涉外调用都过 Sandbox 白名单且都有审计记录；`GET /api/v1/sessions/{id}` 查得到完整对话；同一个 Profile 也能通过 `oryxos chat` 或 `POST /agents/{name}/invoke` 手动补跑一次，验证"人推"和"钟推"复用同一条链路。纯 Profile、不碰能力。

涉及能力一（Provider）+ 能力二（ReAct）+ 能力四（内置 HTTP Tool + `NotifyTools` + Sandbox）+ 定时任务（`AgentScheduler`）+ 能力五（Session 查询兜底）。

### 12.2 Demo 二：每日科技日报（Profile + 能力 L2）

**场景：** 每天早上 9 点，Agent 自动汇总当日科技新闻并推送，日报内容会体现用户之前提过的关注方向。业务方全程不写 Java 代码。

1. 业务方**装一个能力**：`.oryxos/skills/tech-digest/SKILL.md`（frontmatter 只有 `name`+`description`，正文写组稿指令）→ `SkillRegistry` 登记。再**建一个 Agent**：`.oryxos/profiles/daily-tech-digest.yaml`（`identity` + `tools`（含 `use_skill`、`notify`）+ 每天 09:00 的 `schedules` + `notify_channels`）；需要新闻聚合 MCP 就在 `mcp_servers.yaml` 配一条
2. 用户此前说过"更关注 AI 和芯片方向"，DeepSeek 调 `save_memory` 写入 `MEMORY.md` 归档区
3. 到点，`AgentScheduler` 触发 `AgentService.process`，`PromptBuilder` 往 system prompt 注入 `<available_skills>` 索引（含 `tech-digest` 的 name+description，**L1**）+ `MEMORY.md`
4. LLM 按 `description` 命中，调 **`use_skill("tech-digest")`** 取回该能力**正文（L2）**作为工具结果进上下文——正文不预载、只在此刻才占上下文
5. LLM 按正文调新闻工具（`http_get` 或新闻 MCP，`McpToolAdapter` 转发）拉当日科技新闻；因为看到记忆里的偏好，组稿时自然侧重 AI 和芯片方向——OryxOS 不解析任务步骤
6. LLM 调内置 `notify(content="...")` 推送，`ToolExecutor` 写 `tool_invocations`

**验收要点：** 业务方只写一份能力 markdown + 一份 Profile YAML，零 Java；`tool_invocations` 里有 `use_skill("tech-digest")`（渐进式披露生效）；`GET /api/v1/skills` 查得到 `tech-digest`；日报体现记忆偏好；同一个 Agent 可被人工触发验证。

涉及 Skill 能力 L1+L2（`use_skill` 加载正文）+ 能力四方式二（MCP）+ 内置 `NotifyTools` + 能力三（Memory）+ 定时任务。

### 12.3 Demo 三：每日 GitHub 日报（Profile + 能力捆绑脚本 L3）

**场景：** 每天早上 9:30，Agent 自动抓 GitHub 今日与本月热门项目、专挑 AI 相关做总结并推送。这个 Demo 演渐进式披露的**第三级（L3）**：能力里捆绑一个脚本，Agent 跑脚本拿确定性数据。

1. 业务方**装一个带脚本的能力** `.oryxos/skills/github-digest/`：`SKILL.md`（name+description + 正文"跑脚本 → 组三段日报 → notify"）+ `scripts/github_trending.py`（免 token 用 GitHub Search API 按日期区间 + stars 近似 trending）。**建 Agent** `github-daily`：Profile `tools:[shell, notify, use_skill]` + 每天 09:30 的 `schedules`
2. 到点触发，`PromptBuilder` 注入 `<available_skills>`（含 `github-digest`，L1）
3. LLM 命中，调 `use_skill("github-digest")` 取正文（L2）
4. LLM 按正文调 `shell` 跑 `python scripts/github_trending.py`（**L3**）——`Sandbox.enforce` 校验 `shell.allowed_commands` 放行 `python`、`file.allowed_paths` 限定到该 skill 的 `scripts/` 目录；脚本自己请求 `api.github.com` 拿数据、返回 JSON。**脚本产出的 JSON 进上下文、脚本代码不进**，写 `tool_invocations`
5. LLM 用 JSON + 记忆偏好组织三段日报（今日 / 本月 / AI 重点），调 `notify` 推送

> **L3 的信任边界（呼应 11.1 与宪法原则四）**：第 4 步里脚本经 `python` 子进程自己发网络请求，**绕过了 `http_get` 的域名白名单**（白名单只管内置 `http_get` 工具）。所以**装一个带脚本的能力 = 信任这个能力的作者**（与 Anthropic 一致）。核心阶段沙箱对 L3 只做"解释器 + 脚本目录"两道白名单，容器 / 网络隔离留扩展阶段。做 Agent OS 要对这条诚实：能跑第三方能力很强，但信任从"底座"挪到了"能力作者"。

**验收要点：** `tool_invocations` 里同时有 `use_skill("github-digest")` 与 `shell` 跑脚本（L1/L2/L3 全链）；脚本产出进上下文、代码不进；AI 段体现记忆偏好；**同一份 `github-digest` 能力可被另一个"周报"Agent（`cron` 改每周一）直接 `use_skill` 复用**——证明 Skill 是能力、不是 Agent。

涉及 Skill 能力 L1+L2+L3（`use_skill` + 捆绑脚本 + 沙箱信任边界）+ 能力二（ReAct）+ 能力三（Memory）+ 定时任务 + 内置 `NotifyTools`。
