...
- [ ] 有没有把 `SkillLoader` 当成 Tool（它应该归 core 的 `ContextLoader`）
...
| Profile 升级类 | Profile 增加 `skills` 字段 + `mcp_servers` 字段 |
...
业务方写 `.oryxos/skills/daily-pr-digest.md` 描述任务，在 `mcp_servers.yaml` 配置 `github-mcp`（用社区现成的 MCP server），配置一个 Profile 引用这个 Skill + MCP server，Agent 启动后能读 `SKILL.md` 描述、调 `github-mcp` 拉 PR、汇总成简报，整个过程业务方零代码只写了一份 markdown + 配置。
...
| `SkillLoader` 当成 Tool | 应该归 `ContextLoader`，在 core 模块里 |
