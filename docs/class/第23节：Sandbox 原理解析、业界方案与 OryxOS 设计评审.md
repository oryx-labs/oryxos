...
### 九、业界真实系统怎么选沙箱：三类产品对号入座

**把四档从"技术清单"变成"谁在用、为什么用"——你会发现关键不是哪档最强，而是"跑谁的代码"决定选型。**

前面八节讲的是隔离**技术**，这节讲隔离**选型**：同样是 Agent，跑在你自己机器上的编码助手、和跑陌生人代码的云端解释器，选的档差着两级。看三类真实系统，选型逻辑立刻清楚。

**第一类，本地编码 Agent——以 Claude Code 为代表。** 它跑在你自己可信的机器上，威胁模型是"模型误操作 + 提示注入骗它删文件或外传数据"，不是"跑一段蓄意恶意的陌生代码"。所以它的选型是**一档权限 + 二档 OS 轻隔离叠加，不上 microVM**：日常主防线是权限门（每个危险操作问你 `allow` / `ask` / `deny`）；再叠一层可选的 OS 级沙箱（`/sandbox` 一开），macOS 用 Seatbelt（`sandbox-exec`）、Linux 用 bubblewrap 加 seccomp 加 socat 网络代理；把文件写入限制在当前工作目录（你的 SSH 私钥、凭证对 Agent 直接"隐身"），网络只放行白名单里的域名，子进程也继承同一套限制。Anthropic 还把这套抽出来开源成了 `sandbox-runtime`。**这对 OryxOS 直接有参照意义**：Claude Code 的威胁模型和 OryxOS 私有部署最接近（都是跑自己可信的 Agent、重点防误伤与注入，而非跑不可信代码），它没有直奔 microVM，恰恰印证了 OryxOS 走"第一档白名单起步、后续叠 OS 隔离"这条线是对路的。

**第二类，可插拔多后端 Agent——Hermes Agent（NousResearch）值得单独解剖。** 它把"隔离档"做成了可插拔的 terminal backend，一共六种：`local`（直接跑宿主、无隔离）、`Docker`（容器）、`SSH`（远程主机）、`Singularity`（HPC 容器）、`Modal` / `Daytona`（serverless 云沙箱）。

换隔离强度就是换一个后端——这跟 OryxOS 想做的"Sandbox 接口加可换后端"、以及第 22 节 Memory 那三档可插拔存储，是同一套"接口墙"思路。它的安全文档还有一句话，把这一整节的题眼点破了：

> **"对抗恶意 LLM 的唯一安全边界是操作系统。"** 进程内的任何东西——审批门、输出脱敏、模式扫描、工具白名单——都不构成"关押"；任何在进程内筛查 LLM 输出的组件，都只是"作用在一段被攻击者影响过的字符串上的启发式"。

这正是第四节"白名单只是劝阻不是关押"的更硬表述。它进一步区分了两种隔离**姿态**，其中一个区分点最容易被忽略：**terminal-backend isolation** 只把 shell 和文件工具关进沙箱，但**管不住 Agent 自己进程里的东西**——代码执行工具、MCP 子进程、插件 / hook / skill 加载全在进程内，隔离不到；要真正兜住，得用 **whole-process wrapping**，把整个进程树塞进沙箱（用 Docker，或 NVIDIA 的 **OpenShell**——按会话隔离、声明式策略覆盖文件系统 / L7 网络出口 / 系统调用 / 推理路由，凭证从外部注入、不落沙箱磁盘）。**这个坑对 OryxOS 是现成的提醒**：我们的 `Sandbox.enforce` 只拦"工具执行"那一条路径，可 MCP 子进程、进程内直调的内置工具，同样是暴露面——"只隔离了 shell"不等于"隔离了 Agent"。

**第三类，云端代码解释器与 Agent 云沙箱——跑的是陌生人的代码。** OpenAI 的 Code Interpreter 用 **gVisor**（用户态内核容器，在容器之上额外隔离宿主节点）；专门的 Agent 云沙箱平台里，**E2B** 用 Firecracker microVM、**Daytona** 用 gVisor（冷启约 90ms）、**Modal** 甚至能在沙箱里跑 GPU。它们的共同点是：跑完全不可信代码、多租户、要规模化，所以直奔 microVM 这条黄金标准（呼应第六节）。**但这恰恰反衬出 OryxOS 不必上 microVM**——OryxOS 跑的是企业自己可信的 Agent，威胁模型和"跑陌生人代码"根本不同。选型由威胁模型决定，不是越强越好。

一句话收束这三类：**Claude Code（可信机器，防误伤）→ Hermes（可切换，从本地到云沙箱）→ 云解释器（不可信代码，直奔 microVM）**，选型强度跟着"代码可信度"走。OryxOS 站在最左边那一档附近，是想清楚了威胁模型之后的选择，不是能力不足。

### 十、学界与安全社区的共识：为什么白名单挡不住蓄意攻击

**沙箱选型不只是工程问题——学界已经把"为什么审批门和白名单防不住蓄意攻击"讲成了一个有名字的问题。**

- **混淆代理问题（confused deputy）。** Agent 本身握着高权限（能读文件、发请求、调工具），却会被一段低权限的输入——提示注入——诱骗着去滥用这些权限。纯白名单和审批门防不住它，因为它们校验的是"这个请求长什么样、合不合规则"，而不是"这个请求是不是被污染的数据诱导出来的"。工具调用本身完全合法，坏在"为什么调"。
- **提示注入攻防基准：AgentDojo（苏黎世联邦理工 spylab，NeurIPS）。** 它搭了个动态环境，专门量化"会调工具的 Agent 在处理不可信数据时的对抗鲁棒性"——核心命题正是"外部工具返回的数据能劫持 Agent 去执行恶意任务"。它把"注入能不能骗 Agent 越权调工具"变成了可打分的指标，也验证了各种防御（如工具过滤）到底有没有用。
- **能力安全模型（capability-based security）。** 学界更主张按最小权限直接授予"能力令牌"，而不是事后去校验路径字符串——白名单本质是 capability 的一个粗糙近似。
- **一句收束。** 这些讨论共同支撑第八节那个判断：沙箱不是开关，是组合。而且再强的隔离也挡不住"一个被合法授权的工具，在被污染的指令下干坏事"——所以**审计**（记录 Agent 到底干了什么）和**最小权限**（只给它这次任务真正需要的工具）跟隔离本身同等重要。这也正好回接 OryxOS 一贯的两条底线：审计从第一天就落库、Profile 的 `tools` 字段做每个 Agent 的最小工具面。

放到 OryxOS 身上，这两节的结论很诚实：**OryxOS 的第一档白名单不是安全边界，它防的是误操作、限的是可用面。** 真正的对抗性安全，要靠后面 OS 级隔离（第 24 节起）、审计（已 day one 落库）、最小权限（Profile `tools` 字段）三者合力——单靠任何一层都不够，这就是"纵深防御"用在 OryxOS 上的样子。

---

## 第二部分：OryxOS 准备怎么实现 Sandbox

### 十一、设计原则：从第一天就按多阶段演进来设计
...
还有第九节 Hermes 那个坑要一并记住：接口不光要能套进更重的实现，覆盖面也不能只停在"工具执行"这一条路径——将来 MCP 子进程、进程内直调的能力都是暴露面，接口的抽象要留得下"这些也归 Sandbox 管"的空间，别把它焊死成"只拦一次工具调用"。
