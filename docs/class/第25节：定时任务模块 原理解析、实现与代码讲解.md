...
- 配一个 Profile，`schedules` 设成"每分钟触发一次"（方便测试），到点能看到 Agent 自动发起了一次对话，不需要人手动输入。
- 这次触发跟手动触发一样，`llm_calls`/`tool_invocations` 里能查到对应记录。
- 手动模拟"上一次还没跑完"（比如让 Agent 处理一个耗时任务），确认下一次触发被本地锁挡住，不会叠加执行，日志里能看到"skip this trigger"。
- 故意让一次定时任务失败（比如配置一个会报错的 Skill），确认异常被记下来、其他定时任务和系统其他功能都不受影响。
- 改一下 Profile 里的 cron 表达式，不用重新编译、重启后就按新时间跑。
- 配置里显式指定了时区，不是隐式依赖服务器系统时区。
- 端到端预演：拿一个配了 `schedules` 的测试 Profile 完整走一遍"到点自动触发 → 跑完 ReAct 循环 → 留下审计记录"，为 31 节的两个定时 Demo（每日天气、每日科技日报）把地基踩实。
