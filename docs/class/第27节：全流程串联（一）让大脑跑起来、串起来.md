...
## 五、把它跑起来给人看：启动 Web Service 与管理台

前面都在对账"痕迹"，这节第一次要把整台机器**当着人的面跑起来**——而 Web Service 和管理台（web manager）是**同一个进程的两张脸**：`oryxos serve` 起的那个 Spring 同时对外提供 `/api/v1/**` 的 REST 接口和 `/admin` 的管理台页面，不是两个服务、不用起两个进程。

**这是一个开发任务，不是"打开个网页"。** 管理台是 Vue（26 节交付，跟 `website/` 首页同栈 Vue 3 + Vite），它不是随手写的静态 html，而是要**构建**出来、由 Spring 托管：前端 `npm run build` 产物落在 `oryxos-web/src/main/resources/static/admin/`，Spring 把 `/admin` 指过去、并对 `/admin/**` 未命中路径回落 `index.html`（SPA 路由，`/api/v1/**` 不受影响）。所以"启动管理台"= 先把前端 build 进 jar，再 `serve`——这条构建链就是本节要串通并写进 README 的东西。

**前置（不满足就是空转）：** ① `export DEEPSEEK_API_KEY=...`——因为 26 节已排除 `OpenAiAutoConfiguration`，只需这一个 key 就能起（若它还要 `spring.ai.openai.api-key`，回 26 节补排除）；② 前端已构建进 `static/admin/`。

**启动步骤（同一条命令同时给出 REST 和管理台）：**
...
open  http://localhost:8080/admin         # 管理台（Vue）能打开、五页渲染真实数据
open  http://localhost:8080/swagger-ui    # 接口文档
```

**串联对账的收口**：在管理台里点开"会话列表"，看到的应当**正是**刚才 CLI / REST 那次"天气穿衣"对话——同一份数据、同一个引擎的第三个视图（CLI、REST、管理台三面一致）。开发调试时也可以 `npm run dev` 起前端热更、代理到 8080 调后端，但**发布形态是"一个 jar 托管一切"**，README 要按发布形态写。
...
- 同一个工具在一次对话里只被执行一次。
- **Web Service + 管理台一条命令起来**：`serve` 后 `curl /api/v1/health` 通、浏览器开 `http://localhost:8080/admin` 管理台（Vue）五页渲染真实数据，且"会话列表"里就是刚才那次对话——CLI / REST / 管理台三面同源。
